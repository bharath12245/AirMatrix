...
import { predictFlightPrice, PricePrediction } from '@/lib/pricePrediction';
...
const FlightsPage = () => {
  ...
  useEffect(() => {
    if (!searchParams) {
      ...
      // We can't easily set store state here without valid hooks or extensive changes, 
      // but we can just let it render with empty state or defaults if the component handles it.
      // For now, let's just NOT redirect and showing a message or default search.
      // Actually, best is to init search if missing.
    }

    // Better fix: Don't redirect. Let the effect run.
    ...
    const fetchFlights = async () => {
      setLoading(true);
      try {
        const results = await searchFlightsWithAmadeus(effectiveParams);
        setSearchResult(results);
        setFlights(results.flights);

        // Get fare calendar
        const calendar = await getFareCalendarWithHistory(effectiveParams.from, effectiveParams.to);
                ...
                {sortedFlights.map((flight, index) => {
                  ...
                  return (
                    <div key={flight.id} className="relative">
                      ...
                      {(() => {
                        const prediction = predictFlightPrice(flight);
                        let variant: "default" | "secondary" | "outline" | "destructive" = "outline"; // strictly typed
                        let className = "border-0 shadow-lg font-semibold flex items-center gap-1 ";

                        if (prediction.action === 'BUY_NOW') {
                          className += "bg-gradient-to-r from-emerald-500 to-teal-500 text-white";
                        } else if (prediction.action === 'WAIT') {
                        ...
                        return (
                          <div className="absolute -top-2 right-4 z-10">
                            ...
                            >
                              ...
                              {prediction.action === 'BUY_NOW' && "AI: Buy Now"}
                              {prediction.action === 'WAIT' && "AI: Wait"}
                              {prediction.action === 'RISING_SOON' && "AI: Rising Soon"}
                            </Badge>
                          </div>
                        );
                      })()}

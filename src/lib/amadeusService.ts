...
const generateFallbackFlights = (
  ...
  // Helper to parse "2h 30m" -> minutes
  ...
  const addDurationToTime = (timeStr: string, durStr: string) => {
  ...
  for (let i = 0; i < numFlights; i++) {
    ...
    const departureTime = generateTime(departureHour);
    const duration = generateDuration(fromAirport, toAirport, stops);
    const arrivalTime = addDurationToTime(departureTime, duration);
...
export const searchFlightsWithAmadeus = async (params: SearchParams): Promise<AmadeusSearchResult> => {
  ...
  // Reverted to mock/fallback implementation by user request

  // Fallback to Supabase or Mock
  try {
    console.log(`Searching Amadeus (Supabase Proxy): ${fromAirport.code} -> ${toAirport.code} on ${params.date}`);
